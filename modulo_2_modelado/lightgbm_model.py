...
Replica exactamente el protocolo de `xgboost_model.py`:
- Pipeline de imblearn con StandardScaler + (SMOTE opcional) + LGBMClassifier
...
- Umbral óptimo sobre predicciones out-of-fold (OOF) del train, construidas
  manualmente con el mismo `TimeSeriesSplit` interno
...
class LightGBMModel:
    """
    LightGBM con walk-forward CV externo y RandomizedSearchCV interno.
    """
    ...
    def construir_pipeline(self) -> Pipeline:
        ...
        - 'none':     LGBMClassifier sin compensación
        - 'balanced': class_weight='balanced' nativo de LightGBM
        - 'smote':    SMOTE en el pipeline sin class_weight para evitar doble
                      compensación
        """
        # n_jobs=1: evita deadlock por oversubscription entre joblib (n_jobs=-1 en RandomizedSearchCV)
        # y el multithreading interno de LightGBM (OpenMP).
    ...
    def walk_forward_cv(self, X: pd.DataFrame, y: pd.Series, n_folds: int = 4) -> Dict:
        ...
        for i, config in enumerate(fechas_corte):
            ...
            # Umbral OOF: LightGBM satura probabilidades in-sample → umbral no transferible al test.
...
def _ejecutar_target(
    ...
    modelo = LightGBMModel(random_state=42, rebalanceo='balanced')
...
def _ablacion_target(etiqueta: str, X: pd.DataFrame, y: pd.Series) -> Dict:
    ...
    for cond in CONDICIONES_REBALANCEO:
        modelo = LightGBMModel(random_state=42, rebalanceo=cond)
...
if __name__ == "__main__":
    ...
    _ejecutar_target(
        ...
        X=X,
        y=y_6m,
        ruta_metricas="models/lightgbm_metrics_6m.json",
        ruta_roc_pr="docs/figures/lightgbm_6m_roc_pr.png",
        ruta_pr_train="docs/figures/lightgbm_6m_pr_train_por_fold.png",
    )
    ...
    _ejecutar_target(
        ...
        X=X,
        y=y_12m,
        ruta_metricas="models/lightgbm_metrics_12m.json",
        ruta_roc_pr="docs/figures/lightgbm_12m_roc_pr.png",
        ruta_pr_train="docs/figures/lightgbm_12m_pr_train_por_fold.png",
    )
    ...
    with open("models/lightgbm_ablacion.json", 'w') as f:
